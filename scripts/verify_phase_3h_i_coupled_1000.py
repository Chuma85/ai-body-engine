...
DEFAULT_DATASET = "data/synthetic/phase_3h_i_coupled_1000"
DEFAULT_AUDIT_OUT = "artifacts/phase_3h_i_coupled_1000_audit"
DEFAULT_CORRELATION_OUT = "artifacts/phase_3h_i_label_visual_correlation"
DEFAULT_TRAINING_OUT = "artifacts/phase_3h_i_blend_baseline"
DEFAULT_BATCH_OUT = "artifacts/phase_3h_i_generation_batches"
DEFAULT_SMOKE_DATASET = "data/synthetic/phase_3h_i_coupled_smoke"
DEFAULT_SMOKE_BATCH_OUT = "artifacts/phase_3h_i_smoke_generation_batches"
PHASE_3H_H_METRICS = "artifacts/phase_3h_h_blend_baseline/metrics.json"
...
DEFAULT_SAFE_FRAMING_SCALE = 1.12
...
def verify_coupled_1000(
    *,
    ...
    smoke: bool = False,
    phase_3h_h_metrics: str = PHASE_3H_H_METRICS,
) -> dict[str, Any]:
    ...
    dataset_summary = validate_phase_3h_i_dataset(dataset, expected_samples=samples)
    if smoke or not run_benchmark:
        return {
            ...
            "comparison_to_phase_3h_h": {"available": False, "reason": "Benchmark skipped."},
            "commands": {
    ...
    comparison = compare_to_phase_3h_h(training_metrics, phase_3h_h_metrics)
    ...
    return {
        ...
        "comparison_to_phase_3h_h": comparison,
        "commands": {
            "generation": generation_summary["commands"],
    ...
    for chunk_start in range(1, samples + 1, batch_size):
        ...
        if status["complete"]:
            if force and chunk_start >= min_start_index:
                print(f"Phase 3H-I batch {chunk_start}-{end_index}: force enabled; replacing complete chunk.", flush=True)
                shutil.rmtree(chunk_path)
            else:
                print(f"Phase 3H-I batch {chunk_start}-{end_index}: already complete; skipping.", flush=True)
        ...
        if chunk_path.exists() and not status["complete"]:
            if not force:
                raise ValueError(
                    f"Phase 3H-I batch {chunk_start}-{end_index} is incomplete "
                    f"(rows={status['rows']}, pngs={status['pngs']}). Pass --resume --force to replace only incomplete chunks."
                )
            print(f"Phase 3H-I batch {chunk_start}-{end_index}: incomplete; replacing partial chunk.", flush=True)
            shutil.rmtree(chunk_path)
        if chunk_start < min_start_index:
            raise ValueError(f"Phase 3H-I batch {chunk_start}-{end_index} is before --start-index and is not complete.")
        ...
        print(f"Phase 3H-I batch {chunk_start}-{end_index}: rendering {chunk_count} samples.", flush=True)
        subprocess.run(command, cwd=REPO_ROOT, check=True)
        status = inspect_chunk(chunk_path, expected_start=chunk_start, expected_count=chunk_count)
        if not status["complete"]:
            raise ValueError(f"Phase 3H-I batch {chunk_start}-{end_index} did not complete: {status}")
        print(f"Phase 3H-I batch {chunk_start}-{end_index}: complete ({status['rows']} labels, {status['pngs']} PNGs).", flush=True)
        chunk_paths.append(chunk_path)
...
def validate_phase_3h_i_dataset(dataset: str | Path, expected_samples: int = DEFAULT_SAMPLES) -> dict[str, Any]:
    dataset_path = Path(dataset)
    ensure_not_archived_dataset(dataset_path)
    ...
    for path in (dataset_path, labels_path, metadata_path, images_dir, *view_dirs.values()):
        if not path.exists():
            raise FileNotFoundError(f"Missing required Phase 3H-I dataset path: {path}")
    ...
    if clipping["clipped_view_count"] > 0:
        raise ValueError(
            "Phase 3H-I clipping check failed; do not run correlation or training. "
...
def ensure_not_archived_dataset(dataset: str | Path) -> None:
    ...
    if any(marker in normalized for marker in ARCHIVED_DATASET_MARKERS):
        raise ValueError(f"Phase 3H-I must not use archived old datasets: {dataset}")
...
def compare_to_phase_3h_h(metrics: dict[str, Any], phase_3h_h_metrics: str | Path = PHASE_3H_H_METRICS) -> dict[str, Any]:
    baseline_path = Path(phase_3h_h_metrics)
    if not baseline_path.exists():
        return {
            "available": False,
            "baseline_metrics": str(baseline_path),
            "reason": "Phase 3H-H metrics report was not found.",
        }
    ...
    for target in TARGET_COLUMNS:
        ...
        target_comparisons[target] = {
            "phase_3h_h_mae": old_value,
            "phase_3h_i_mae": new_value,
            "delta": delta,
    ...
    return {
        "available": True,
        "phase_3h_h_best_model": baseline.get("best_model"),
        "phase_3h_h_overall_mean_mae": float(baseline["overall_mean_mae"]),
        "phase_3h_i_overall_mean_mae": float(metrics["overall_mean_mae"]),
...
def format_summary(summary: dict[str, Any]) -> str:
    ...
    lines = [
        "Phase 3H-I coupled verification passed.",
    ...
    comparison = summary["comparison_to_phase_3h_h"]
    if comparison.get("available"):
        lines.append(
            "Comparison vs Phase 3H-H: "
            f"{comparison['overall_status']} ({comparison['overall_delta']:.4f} cm)"
        )
    else:
        lines.append(f"Comparison vs Phase 3H-H: unavailable ({comparison['reason']})")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate and benchmark the Phase 3H-I coupled 1000-sample Blender dataset.")
    ...
    parser.add_argument("--phase-3h-h-metrics", default=PHASE_3H_H_METRICS)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.smoke:
        ...
        if audit_out == DEFAULT_AUDIT_OUT:
            audit_out = "artifacts/phase_3h_i_smoke_audit"
        if correlation_out == DEFAULT_CORRELATION_OUT:
            correlation_out = "artifacts/phase_3h_i_smoke_label_visual_correlation"
        if training_out == DEFAULT_TRAINING_OUT:
            training_out = "artifacts/phase_3h_i_smoke_blend_baseline"
        resume = True
    try:
        summary = verify_coupled_1000(
            ...
            smoke=args.smoke,
            phase_3h_h_metrics=args.phase_3h_h_metrics,
        )
    except (RuntimeError, ValueError, FileNotFoundError, subprocess.CalledProcessError) as exc:
        print(f"Phase 3H-I coupled 1000 verification failed: {exc}")
        return 1
